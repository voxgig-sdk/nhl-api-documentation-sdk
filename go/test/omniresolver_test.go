...
import (
	...
	omni "github.com/voxgig-sdk/nhl-api-documentation-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NhlApiDocumentationError); is {
				out := map[string]any{
					"name":    "NhlApiDocumentationError",
...
func omniCtx(arg any, client *sdk.NhlApiDocumentationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NhlApiDocumentationSDK); is {
			client = live
