...
func TestDivisionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Division(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Division(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		divisionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.division")))
		...
		divisionRef01DataDt0LoadResult := core.ToMapAny(entityData(divisionRef01DataDt0Loaded))
...
func divisionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NHL_API_DOCUMENTATION_TEST_DIVISION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NHL_API_DOCUMENTATION_TEST_DIVISION_ENTID"])
