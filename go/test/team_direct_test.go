...
func TestTeamDirect(t *testing.T) {
	t.Run("direct-list-team", func(t *testing.T) {
		setup := teamDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-team", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-team", func(t *testing.T) {
		setup := teamDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-team", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type teamDirectSetupResult struct {
...
func teamDirectSetup(mockres any) *teamDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"NHL_API_DOCUMENTATION_TEST_TEAM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["NHL_API_DOCUMENTATION_TEST_TEAM_ENTID"]; ok {
		...
		return &teamDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &teamDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
