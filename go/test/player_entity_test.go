...
func TestPlayerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		playerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.player")))
		...
		playerRef01MatchDt0 := map[string]any{
			"id": playerRef01Data["id"],
		}
		playerRef01DataDt0Loaded, err := playerRef01Ent.Load(playerRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		playerRef01DataDt0LoadResult := core.ToMapAny(entityData(playerRef01DataDt0Loaded))
		if playerRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if playerRef01DataDt0LoadResult["id"] != playerRef01Data["id"] {
...
func playerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID"])
