...
func NewPlayerEntity(client *core.NhlApiDocumentationSDK, entopts map[string]any) *PlayerEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
// argument it returns the current data as an Player; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *PlayerEntity) DataTyped(data ...Player) Player {
	if len(data) > 0 {
		return typedFrom[Player](e.Data(asMap(data[0])))
	}
	return typedFrom[Player](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Player (all fields
// optional at the wire level).
func (e *PlayerEntity) MatchTyped(match ...Player) Player {
	if len(match) > 0 {
		return typedFrom[Player](e.Match(asMap(match[0])))
	}
	return typedFrom[Player](e.Match())
}
...
func (e *PlayerEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
// PlayerLoadMatch and returns an Player. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *PlayerEntity) LoadTyped(reqmatch PlayerLoadMatch, ctrl map[string]any) (Player, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return Player{}, err
	}
	return typedFrom[Player](res), nil
}
