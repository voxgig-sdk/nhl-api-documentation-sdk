...
class DivisionEntity
  ...
  def make
    opts = @_entopts.dup
    DivisionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NhlApiDocumentationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Division, Hash] the current Division data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NhlApiDocumentationHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Division fields)
  def match_get
  ...
  #
  # @param reqmatch [DivisionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Division.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Division, Hash] the loaded Division; raises NhlApiDocumentationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = NhlApiDocumentationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [DivisionListMatch, Hash, nil] match filter (any subset of
  #   Division fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Division>, Array] the matching Division items; raises NhlApiDocumentationError on failure
