...
class PlayerDirectTest < Minitest::Test
  def test_direct_load_player
    setup = player_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-player", setup[:live] ? "live" : "unit")
    if _should_skip
...
def player_direct_setup(mockres)
