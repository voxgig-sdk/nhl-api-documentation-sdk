...
class PlayerEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    player_ref01_data_dt0_load_result = Helpers.to_map(player_ref01_data_dt0_loaded.respond_to?(:data_get) ? player_ref01_data_dt0_loaded.data_get : player_ref01_data_dt0_loaded)
    assert !player_ref01_data_dt0_load_result.nil?
    assert_equal player_ref01_data_dt0_load_result["id"], player_ref01_data["id"]
...
def player_basic_setup(extra)
  ...
  entid_env_raw = ENV["NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID"]
