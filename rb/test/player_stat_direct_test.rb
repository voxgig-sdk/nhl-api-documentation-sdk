...
class PlayerStatDirectTest < Minitest::Test
  def test_direct_list_player_stat
    setup = player_stat_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-player_stat", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["person01"].each do |_live_key|
...
def player_stat_direct_setup(mockres)
