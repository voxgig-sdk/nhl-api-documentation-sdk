...
class ConferenceEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Conference(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Conference(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = conference_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "conference." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NHL_API_DOCUMENTATION_TEST_CONFERENCE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    conference_ref01_data = nil
    if conference_ref01_data_raw.length > 0
    ...
    conference_ref01_data_dt0_load_result = Helpers.to_map(conference_ref01_data_dt0_loaded.respond_to?(:data_get) ? conference_ref01_data_dt0_loaded.data_get : conference_ref01_data_dt0_loaded)
...
def conference_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "conference", "ConferenceTestData.json")
  ...
  entid_env_raw = ENV["NHL_API_DOCUMENTATION_TEST_CONFERENCE_ENTID"]
