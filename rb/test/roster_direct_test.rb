...
class RosterDirectTest < Minitest::Test
  def test_direct_list_roster
    setup = roster_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-roster", setup[:live] ? "live" : "unit")
    if _should_skip
...
def roster_direct_setup(mockres)
