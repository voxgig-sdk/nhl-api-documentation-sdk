...
class StandingDirectTest < Minitest::Test
  def test_direct_list_standing
    setup = standing_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-standing", setup[:live] ? "live" : "unit")
    if _should_skip
...
def standing_direct_setup(mockres)
