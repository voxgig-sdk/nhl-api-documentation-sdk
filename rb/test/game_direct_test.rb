...
class GameDirectTest < Minitest::Test
  def test_direct_load_game
    setup = game_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-game", setup[:live] ? "live" : "unit")
    if _should_skip
