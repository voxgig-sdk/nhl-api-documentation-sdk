...
class DivisionEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.Division(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Division(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    setup = division_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["list", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "division." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set NHL_API_DOCUMENTATION_TEST_DIVISION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    division_ref01_data = nil
    if division_ref01_data_raw.length > 0
    ...
    division_ref01_data_dt0_load_result = Helpers.to_map(division_ref01_data_dt0_loaded.respond_to?(:data_get) ? division_ref01_data_dt0_loaded.data_get : division_ref01_data_dt0_loaded)
...
def division_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "division", "DivisionTestData.json")
  ...
  entid_env_raw = ENV["NHL_API_DOCUMENTATION_TEST_DIVISION_ENTID"]
