...
class ScheduleDirectTest < Minitest::Test
  def test_direct_list_schedule
    setup = schedule_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-schedule", setup[:live] ? "live" : "unit")
    if _should_skip
...
def schedule_direct_setup(mockres)
