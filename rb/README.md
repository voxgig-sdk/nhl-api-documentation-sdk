...
The Ruby SDK for the NhlApiDocumentation API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Conference(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.NhlApiDocumentation(nil).load(
  { "id" => "test01" }, nil
)
