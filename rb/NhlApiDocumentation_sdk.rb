...
class NhlApiDocumentationSDK
    ...
    if feature_opts
      ...
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NhlApiDocumentationHelpers.to_map(feature_opts[fname])
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => NhlApiDocumentationError.new(
        "#{op}_allow",
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NhlApiDocumentationError.new(
        "graphql_error", "NhlApiDocumentationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
