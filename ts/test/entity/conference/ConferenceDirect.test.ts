...
import { NhlApiDocumentationSDK } from '../../..'
...
describe('ConferenceDirect', async () => {
  ...
  test('direct-load-conference', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-conference', setup.live)) return
  ...
  test('direct-list-conference', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-conference', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NHL_API_DOCUMENTATION_TEST_CONFERENCE_ENTID']
