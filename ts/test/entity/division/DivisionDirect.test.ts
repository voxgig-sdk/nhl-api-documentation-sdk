...
describe('DivisionDirect', async () => {
  ...
  test('direct-load-division', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-division', setup.live)) return
  ...
  test('direct-list-division', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-division', setup.live)) return
    const { client, calls } = setup
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NHL_API_DOCUMENTATION_TEST_DIVISION_ENTID']
