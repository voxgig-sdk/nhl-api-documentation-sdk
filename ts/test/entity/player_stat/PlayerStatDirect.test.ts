...
describe('PlayerStatDirect', async () => {
  ...
  test('direct-list-player_stat', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-player_stat', setup.live)) return
    if (skipIfMissingIds(t, setup, ["person01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['NHL_API_DOCUMENTATION_TEST_PLAYER_STAT_ENTID']
