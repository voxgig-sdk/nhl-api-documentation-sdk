...
//     (\`new NhlApiDocumentationSDK(...)\` / \`NhlApiDocumentationSDK.test(...)\` become
//     \`NhlApiDocumentationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'NhlApiDocumentationSDK'
...
const TEST_SEED = {"entity":{"conference":{"test01":{"id":"test01"}},"division":{"test01":{"id":"test01"}},"game":{"test01":{"id":"test01"}},"player":{"test01":{"id":"test01"}},"player_stat":{"test01":{"id":"test01"}},"roster":{"test01":{"id":"test01"}},"schedule":{"test01":{"id":"test01"}},"standing":{"test01":{"id":"test01"}},"team":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new NhlApiDocumentationSDK(...)\` and any
// \`NhlApiDocumentationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+NhlApiDocumentationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/NhlApiDocumentationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
