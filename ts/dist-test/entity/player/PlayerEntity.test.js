...
(0, node_test_1.describe)('PlayerEntity', async () => {
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        for (const op of ['load']) {
            if ((0, utility_1.maybeSkipControl)(t, 'entityOp', 'player.' + op, live))
                return;
        }
        ...
        if (setup.syntheticOnly) {
            t.skip('live entity test uses synthetic IDs from fixture — set NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID JSON to run live');
            return;
        }
        ...
        player_ref01_match_dt0.id = player_ref01_data.id;
        const player_ref01_data_dt0 = (await player_ref01_ent.load(player_ref01_match_dt0)).data();
        (0, node_assert_1.default)(player_ref01_data_dt0.id === player_ref01_data.id);
    });
});
function basicSetup(extra) {
    ...
    const idmapEnvVal = process.env['NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID'];
    ...
    idmap = env['NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID'];
