...
//    only - a typed NhlApiDocumentationContext there would make every ctx
...
final class NhlApiDocumentationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NhlApiDocumentationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NhlApiDocumentationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NhlApiDocumentationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NhlApiDocumentationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NhlApiDocumentationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NhlApiDocumentationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NhlApiDocumentationError(is_string($code) ? $code : '', $msg);
    }
