...
// NhlApiDocumentation SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = NhlApiDocumentationOmni::makeStructRunner(
    ...
    {
        return NhlApiDocumentationOmni::objmodel(NhlApiDocumentationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return NhlApiDocumentationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return NhlApiDocumentationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(NhlApiDocumentationOmni::structfix($now, Struct::undef()))
                        !== json_encode(NhlApiDocumentationOmni::structfix($val, Struct::undef()))) {
