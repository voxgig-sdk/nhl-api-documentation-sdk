...
// NhlApiDocumentation SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NhlApiDocumentationTestFeature extends NhlApiDocumentationBaseFeature
{
    ...
    public function init(NhlApiDocumentationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NhlApiDocumentationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NhlApiDocumentationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NhlApiDocumentationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NhlApiDocumentationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NhlApiDocumentationParam::call($ctx, $k);
