...
module NhlApiDocumentationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NhlApiDocumentationResponse.new({})
    ...
    if fetch_err
      ...
      response = NhlApiDocumentationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NhlApiDocumentationResponse.new(fetched)
    else
