...
boru-driven command-line client **and** interactive REPL for the NhlApiDocumentation
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/nhl-api-documentation-cli)
make build

# 2. See usage (words, entities, env vars)
./nhl-api-documentation-cli --help
...
./nhl-api-documentation-cli load 1 conference            # {id:1} shorthand
./nhl-api-documentation-cli load '{id:1}' conference       # explicit match map
./nhl-api-documentation-cli list division

# 5. Override the API base URL for a single call
NHL_API_DOCUMENTATION_BASE=https://api.example.com ./nhl-api-documentation-cli list conference

# 6. No arguments -> interactive REPL
./nhl-api-documentation-cli
nhl-api-documentation> list conference
nhl-api-documentation> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/nhl-api-documentation-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/nhl-api-documentation-cli list conference
   ```
...
```sh
./nhl-api-documentation-cli load 1 conference          # scalar shorthand for {id:1}
./nhl-api-documentation-cli load '{id:1}' conference     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `nhl-api-documentation>`). Each line is
evaluated as its own boru expression:

```text
$ ./nhl-api-documentation-cli
nhl-api-documentation> list conference
nhl-api-documentation> /help
nhl-api-documentation> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `conference`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/nhl-api-documentation-cli`. |
...
(`./nhl-api-documentation-cli <expr>`) and interactively (the REPL), and expressions compose the
