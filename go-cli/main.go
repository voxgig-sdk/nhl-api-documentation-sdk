// Command NhlApiDocumentation-cli is a CLI + REPL for the NhlApiDocumentation SDK,
...
const prompt = "nhl-api-documentation"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "conference division game player player_stat roster schedule standing team"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NHL_API_DOCUMENTATION_APIKEY (api key), NHL_API_DOCUMENTATION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NHL_API_DOCUMENTATION_APIKEY carries the API key and
	// NHL_API_DOCUMENTATION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NHL_API_DOCUMENTATION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NHL_API_DOCUMENTATION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNhlApiDocumentationSDK(opts)
