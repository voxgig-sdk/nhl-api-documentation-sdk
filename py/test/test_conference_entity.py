...
class TestConferenceEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Conference(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Conference(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set NHL_API_DOCUMENTATION_TEST_CONFERENCE_ENTID JSON to run live")
        client = setup["client"]
        ...
        conference_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(conference_ref01_data_dt0_loaded))
...
def _conference_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("NHL_API_DOCUMENTATION_TEST_CONFERENCE_ENTID"))
