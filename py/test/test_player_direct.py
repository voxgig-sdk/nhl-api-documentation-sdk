...
class TestPlayerDirect:

    def test_should_direct_load_player(self):
        setup = _player_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-player", "live" if setup["live"] else "unit")
        if _skip:
...
def _player_direct_setup(mockres):
