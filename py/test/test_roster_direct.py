...
class TestRosterDirect:

    def test_should_direct_list_roster(self):
        setup = _roster_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-roster", "live" if setup["live"] else "unit")
        if _skip:
...
def _roster_direct_setup(mockres):
