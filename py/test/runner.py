# NhlApiDocumentation SDK test runner
...
class NhlApiDocumentationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NhlApiDocumentationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NhlApiDocumentationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NhlApiDocumentationTestRunner.getenv("NHL_API_DOCUMENTATION_TEST_LIVE")
        override = NhlApiDocumentationTestRunner.getenv("NHL_API_DOCUMENTATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NhlApiDocumentationTestRunner.getenv(key)
        ...
        explain = NhlApiDocumentationTestRunner.getenv("NHL_API_DOCUMENTATION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NHL_API_DOCUMENTATION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NhlApiDocumentationTestRunner._test_control is not None:
            return NhlApiDocumentationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NhlApiDocumentationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NhlApiDocumentationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NhlApiDocumentationTestRunner.load_test_control()
...
def load_env_local():
    NhlApiDocumentationTestRunner.load_env_local()


def env_override(m):
    return NhlApiDocumentationTestRunner.env_override(m)


def entity_data(v):
    return NhlApiDocumentationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NhlApiDocumentationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NhlApiDocumentationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NhlApiDocumentationTestRunner.load_test_control()


def live_client_options():
    return NhlApiDocumentationTestRunner.live_client_options()


def live_delay_ms():
    return NhlApiDocumentationTestRunner.live_delay_ms()
