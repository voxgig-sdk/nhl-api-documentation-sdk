...
class TestStandingDirect:

    def test_should_direct_list_standing(self):
        setup = _standing_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-standing", "live" if setup["live"] else "unit")
        if _skip:
...
def _standing_direct_setup(mockres):
