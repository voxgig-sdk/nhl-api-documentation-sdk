...
The Python SDK for the NhlApiDocumentation API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Conference(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Conference(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.NhlApiDocumentation(None).load(
    {"id": "test01"}, None
)
