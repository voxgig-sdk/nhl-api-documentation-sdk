...
The Lua SDK for the NhlApiDocumentation API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Conference(nil):list(nil, nil)
...
```lua
local result, err = client:Conference(nil):load({ id = "example_id" }, nil)
if err then error(err) end
print(result)
```
...
local result, err = client:NhlApiDocumentation(nil):load(
  { id = "test01" }, nil
)
