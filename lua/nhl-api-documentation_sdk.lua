...
require("nhl-api-documentation_types")
...
local NhlApiDocumentationSDK = {}
NhlApiDocumentationSDK.__index = NhlApiDocumentationSDK
...
NhlApiDocumentationSDK._make_feature = _make_feature


function NhlApiDocumentationSDK.new(options)
  local self = setmetatable({}, NhlApiDocumentationSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function NhlApiDocumentationSDK:options_map()
...
function NhlApiDocumentationSDK:get_utility()
  return Utility.copy(self._utility)
end


function NhlApiDocumentationSDK:get_root_ctx()
  return self._rootctx
end


function NhlApiDocumentationSDK:prepare(fetchargs)
...
function NhlApiDocumentationSDK:direct(fetchargs)
...
function NhlApiDocumentationSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function NhlApiDocumentationSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "NhlApiDocumentationSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function NhlApiDocumentationSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function NhlApiDocumentationSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "NhlApiDocumentationSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function NhlApiDocumentationSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = NhlApiDocumentationSDK.new(sdkopts)
  sdk.mode = "test"
