...
describe("ConferenceDirect", function()
  it("should direct-list-conference", function()
    local setup = conference_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-conference", setup.live and "live" or "unit")
  ...
  it("should direct-load-conference", function()
    local setup = conference_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-conference", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function conference_direct_setup(mockres)
