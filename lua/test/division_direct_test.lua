...
describe("DivisionDirect", function()
  it("should direct-list-division", function()
    local setup = division_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-division", setup.live and "live" or "unit")
  ...
  it("should direct-load-division", function()
    local setup = division_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-division", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function division_direct_setup(mockres)
