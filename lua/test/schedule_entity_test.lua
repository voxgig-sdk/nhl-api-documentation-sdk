...
describe("ScheduleEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Schedule(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Schedule(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set NHL_API_DOCUMENTATION_TEST_SCHEDULE_ENTID JSON to run live")
      return
    end
    ...
    -- LIST
    local schedule_ref01_ent = client:Schedule(nil)
    local schedule_ref01_match = {}

    local schedule_ref01_list_result, err = schedule_ref01_ent:list(schedule_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(schedule_ref01_list_result)
...
function schedule_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("NHL_API_DOCUMENTATION_TEST_SCHEDULE_ENTID")
  local idmap_overridden = entid_env_raw ~= nil and entid_env_raw:match("^%s*{") ~= nil

  local env = runner.env_override({
    ["NHL_API_DOCUMENTATION_TEST_SCHEDULE_ENTID"] = idmap,
    ["NHL_API_DOCUMENTATION_TEST_LIVE"] = "FALSE",
    ["NHL_API_DOCUMENTATION_TEST_EXPLAIN"] = "FALSE",
  })

  local idmap_resolved = helpers.to_map(
    env["NHL_API_DOCUMENTATION_TEST_SCHEDULE_ENTID"])
