...
local RosterEntity = {}
RosterEntity.__index = RosterEntity


function RosterEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RosterEntity)
  self._name = "roster"
  self._client = client
...
function RosterEntity:get_name()
  return self._name
end


function RosterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RosterEntity.new(self._client, opts)
end
...
function RosterEntity:mark_deleted()
  self._deleted = true
end


function RosterEntity:deleted()
  return true == self._deleted
end


function RosterEntity:data_set(args)
  if args ~= nil then
...
function RosterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RosterEntity:match_set(args)
  if args ~= nil then
...
function RosterEntity:match_get()
...
function RosterEntity:stream(action, args, callopts)
...
---@param reqmatch RosterListMatch
---@param ctrl? table
---@return Roster[]
---@return string? err
function RosterEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function RosterEntity:_run_op(ctx, post_done)
  local utility = self._utility
