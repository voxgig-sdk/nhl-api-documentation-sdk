...
local StandingEntity = {}
StandingEntity.__index = StandingEntity


function StandingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StandingEntity)
  self._name = "standing"
  self._client = client
...
function StandingEntity:get_name()
  return self._name
end


function StandingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StandingEntity.new(self._client, opts)
end
...
function StandingEntity:mark_deleted()
  self._deleted = true
end


function StandingEntity:deleted()
  return true == self._deleted
end


function StandingEntity:data_set(args)
  if args ~= nil then
...
function StandingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StandingEntity:match_set(args)
  if args ~= nil then
...
function StandingEntity:match_get()
...
function StandingEntity:stream(action, args, callopts)
...
---@param reqmatch StandingListMatch
---@param ctrl? table
---@return Standing[]
---@return string? err
function StandingEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function StandingEntity:_run_op(ctx, post_done)
  local utility = self._utility
