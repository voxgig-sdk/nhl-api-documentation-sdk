...
function PlayerEntity.new(client, entopts)
  entopts = entopts or {}
...
function PlayerEntity:get_name()
  return self._name
end


function PlayerEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlayerEntity.new(self._client, opts)
end
...
function PlayerEntity:mark_deleted()
  self._deleted = true
end


function PlayerEntity:deleted()
  return true == self._deleted
end


function PlayerEntity:data_set(args)
  if args ~= nil then
...
function PlayerEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlayerEntity:match_set(args)
  if args ~= nil then
...
function PlayerEntity:match_get()
...
function PlayerEntity:stream(action, args, callopts)
...
---@param reqmatch PlayerLoadMatch
---@param ctrl? table
---@return Player
---@return string? err
function PlayerEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function PlayerEntity:_run_op(ctx, post_done)
  local utility = self._utility
