-- NhlApiDocumentation SDK PlayerStat entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PlayerStatEntity = {}
PlayerStatEntity.__index = PlayerStatEntity


function PlayerStatEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PlayerStatEntity)
  self._name = "player_stat"
  self._client = client
...
function PlayerStatEntity:get_name()
  return self._name
end


function PlayerStatEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PlayerStatEntity.new(self._client, opts)
end
...
function PlayerStatEntity:mark_deleted()
  self._deleted = true
end


function PlayerStatEntity:deleted()
  return true == self._deleted
end


function PlayerStatEntity:data_set(args)
  if args ~= nil then
...
function PlayerStatEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PlayerStatEntity:match_set(args)
  if args ~= nil then
...
function PlayerStatEntity:match_get()
...
function PlayerStatEntity:stream(action, args, callopts)
...
function PlayerStatEntity:_run_op(ctx, post_done)
  local utility = self._utility
