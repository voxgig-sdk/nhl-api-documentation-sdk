...
local TeamEntity = {}
TeamEntity.__index = TeamEntity


function TeamEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TeamEntity)
  self._name = "team"
  self._client = client
...
function TeamEntity:get_name()
  return self._name
end


function TeamEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TeamEntity.new(self._client, opts)
end
...
function TeamEntity:mark_deleted()
  self._deleted = true
end


function TeamEntity:deleted()
  return true == self._deleted
end


function TeamEntity:data_set(args)
  if args ~= nil then
...
function TeamEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TeamEntity:match_set(args)
  if args ~= nil then
...
function TeamEntity:stream(action, args, callopts)
...
---@param reqmatch TeamLoadMatch
---@param ctrl? table
---@return Team
---@return string? err
function TeamEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function TeamEntity:_run_op(ctx, post_done)
  local utility = self._utility
