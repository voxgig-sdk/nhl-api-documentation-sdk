...
local ScheduleEntity = {}
ScheduleEntity.__index = ScheduleEntity


function ScheduleEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ScheduleEntity)
  self._name = "schedule"
  self._client = client
...
function ScheduleEntity:get_name()
  return self._name
end


function ScheduleEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ScheduleEntity.new(self._client, opts)
end
...
function ScheduleEntity:mark_deleted()
  self._deleted = true
end


function ScheduleEntity:deleted()
  return true == self._deleted
end


function ScheduleEntity:data_set(args)
  if args ~= nil then
...
function ScheduleEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ScheduleEntity:match_set(args)
  if args ~= nil then
...
function ScheduleEntity:match_get()
...
function ScheduleEntity:stream(action, args, callopts)
...
function ScheduleEntity:_run_op(ctx, post_done)
  local utility = self._utility
