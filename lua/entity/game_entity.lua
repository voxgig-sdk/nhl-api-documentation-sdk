...
function GameEntity.new(client, entopts)
  entopts = entopts or {}
...
function GameEntity:get_name()
  return self._name
end


function GameEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GameEntity.new(self._client, opts)
end
...
function GameEntity:mark_deleted()
  self._deleted = true
end


function GameEntity:deleted()
  return true == self._deleted
end


function GameEntity:data_set(args)
  if args ~= nil then
...
function GameEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GameEntity:match_set(args)
  if args ~= nil then
...
function GameEntity:stream(action, args, callopts)
...
function GameEntity:_run_op(ctx, post_done)
  local utility = self._utility
