...
local DivisionEntity = {}
DivisionEntity.__index = DivisionEntity


function DivisionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DivisionEntity)
  self._name = "division"
  self._client = client
...
function DivisionEntity:get_name()
  return self._name
end


function DivisionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DivisionEntity.new(self._client, opts)
end
...
function DivisionEntity:mark_deleted()
  self._deleted = true
end


function DivisionEntity:deleted()
  return true == self._deleted
end


function DivisionEntity:data_set(args)
  if args ~= nil then
...
function DivisionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DivisionEntity:match_set(args)
  if args ~= nil then
...
function DivisionEntity:match_get()
...
function DivisionEntity:stream(action, args, callopts)
...
---@param reqmatch DivisionLoadMatch
---@param ctrl? table
---@return Division
---@return string? err
function DivisionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch DivisionListMatch
---@param ctrl? table
---@return Division[]
---@return string? err
function DivisionEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function DivisionEntity:_run_op(ctx, post_done)
  local utility = self._utility
