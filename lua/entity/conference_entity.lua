...
local ConferenceEntity = {}
ConferenceEntity.__index = ConferenceEntity


function ConferenceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ConferenceEntity)
  self._name = "conference"
  self._client = client
...
function ConferenceEntity:get_name()
  return self._name
end


function ConferenceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ConferenceEntity.new(self._client, opts)
end
...
function ConferenceEntity:mark_deleted()
  self._deleted = true
end


function ConferenceEntity:deleted()
  return true == self._deleted
end


function ConferenceEntity:data_set(args)
  if args ~= nil then
...
function ConferenceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ConferenceEntity:match_set(args)
  if args ~= nil then
...
function ConferenceEntity:match_get()
...
function ConferenceEntity:stream(action, args, callopts)
...
function ConferenceEntity:_run_op(ctx, post_done)
  local utility = self._utility
