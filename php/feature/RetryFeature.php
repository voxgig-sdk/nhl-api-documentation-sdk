...
// NhlApiDocumentation SDK retry feature
...
class NhlApiDocumentationRetryFeature extends NhlApiDocumentationBaseFeature
{
    ...
    public function init(NhlApiDocumentationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NhlApiDocumentationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NhlApiDocumentationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NhlApiDocumentationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
