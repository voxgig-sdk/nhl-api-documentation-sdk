...
class DivisionEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $seen = iterator_to_array($base->Division(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Division(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NHL_API_DOCUMENTATION_TEST_DIVISION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $division_ref01_data_dt0_load_result = Helpers::to_map(is_object($division_ref01_data_dt0_loaded) && method_exists($division_ref01_data_dt0_loaded, 'data_get') ? $division_ref01_data_dt0_loaded->data_get() : $division_ref01_data_dt0_loaded);
...
function division_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("NHL_API_DOCUMENTATION_TEST_DIVISION_ENTID");
