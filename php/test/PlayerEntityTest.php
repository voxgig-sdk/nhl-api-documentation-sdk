...
class PlayerEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function player_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["player01", "player02", "player03"] as $k) {
    ...
    $entid_env_raw = getenv("NHL_API_DOCUMENTATION_TEST_PLAYER_ENTID");
