...
// NhlApiDocumentation SDK feature test
//
...
class FtCtrl extends NhlApiDocumentationControl
{
...
class FtClient
{
    ...
    private NhlApiDocumentationUtility $utility;

    public function __construct(NhlApiDocumentationUtility $utility, array $options)
    {
    ...
    public function get_utility(): NhlApiDocumentationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NhlApiDocumentationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NhlApiDocumentationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NhlApiDocumentationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NhlApiDocumentationConfig::make_feature($name);
    ...
    private static function buildUrl(NhlApiDocumentationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NhlApiDocumentationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NhlApiDocumentationResponse($fetched);
        }
    ...
    private function fail(NhlApiDocumentationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NhlApiDocumentationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
