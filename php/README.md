...
The PHP SDK for the NhlApiDocumentation API. Provides an entity-oriented interface using PHP conventions.
...
```php
[$result, $err] = $client->Conference(null)->list(null, null);
...
```php
[$result, $err] = $client->Conference(null)->load(["id" => "example_id"], null);
if ($err) { throw new \Exception($err); }
print_r($result);
```
...
[$result, $err] = $client->NhlApiDocumentation(null)->load(
    ["id" => "test01"], null
);
