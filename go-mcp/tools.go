...
type Args struct {
	Entity string         `json:"entity" jsonschema:"conference | division | game | player | player_stat | roster | schedule | standing | team"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.NhlApiDocumentationSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "nhl-api-documentation_list",
		Description: "List records from NhlApiDocumentation. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "nhl-api-documentation_load",
		Description: "Load a single record from NhlApiDocumentation. " +
...
func runOp(client *sdk.NhlApiDocumentationSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.NhlApiDocumentationSDK, name string) (sdk.NhlApiDocumentationEntity, error) {
	switch strings.ToLower(name) {
	case "conference":
		return client.Conference(nil), nil
	case "division":
		return client.Division(nil), nil
	case "game":
		return client.Game(nil), nil
	case "player":
		return client.Player(nil), nil
	case "player_stat":
		return client.PlayerStat(nil), nil
	case "roster":
		return client.Roster(nil), nil
	case "schedule":
		return client.Schedule(nil), nil
	case "standing":
		return client.Standing(nil), nil
	case "team":
